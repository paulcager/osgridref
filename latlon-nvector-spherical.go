 ...
 */
func (ll LatLon) toNVector() NvectorSpherical { // note: replicated in LatLon_NvectorEllipsoidal
	φ := ll.Lat * toRadians
	λ := ll.Lon * toRadians
	...
	return NvectorSpherical{X: x, Y: y, Z: z}
}
...
func (ll LatLon) IsEnclosedBy(polygon []LatLon) bool {
	...
	Σθ := 0.0
	for v := 0; v < nVertices; v++ {
		Σθ += Vector3d(vectorToVertex[v]).AngleTo(Vector3d(vectorToVertex[v+1]), true, Vector3d(p))
	}
